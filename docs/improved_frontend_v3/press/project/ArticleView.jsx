...
const TABS = [
  ...
  { id: "mindmap", label: "Mind map", icon: "share" },
  { id: "shot", label: "Screenshot", icon: "image" },
  { id: "pdf", label: "PDF", icon: "doc" },
  { id: "md", label: "Markdown", icon: "code" },
];

function MindmapTab({ a, onGenerate }) {
  return (
    <div style={{ position: "relative", height: "calc(100vh - var(--topbar-h) - 46px)" }}>
      <div style={{ position: "absolute", left: 20, top: 14, zIndex: 6, display: "flex", alignItems: "center", gap: 10 }}>
        <span className="badge" style={{ background: "oklch(0.72 0.16 150 / 16%)", color: "var(--ok)", fontSize: 10.5 }}><span className="dot" style={{ background: "var(--ok)" }}></span>built during processing</span>
        <Button variant="outline" size="sm" onClick={() => onGenerate && onGenerate("mindmap", a.title)}><Icon name="sparkles" size={13} />Customize</Button>
      </div>
      <Mindmap tree={window.mindmapFor(a)} />
    </div>
  );
}

function ArticleView({ article, onBack, onGenerate }) {
  ...
  return (
    <div className="av-wrap">
      <div className="av-main">
        ...
        {tab === "mindmap" && <MindmapTab a={article} onGenerate={onGenerate} />}
